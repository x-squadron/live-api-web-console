...
import { linearGetIssues, slackSendMessage } from "./tools/composioTools.js";
...
export class MeetingSummarizer {
  constructor() {
    ...
    this.analysisAgent = createReactAgent({
      llm: this.llm,
      tools: [linearGetIssues, slackSendMessage],
...
- Focus ONLY on Linear-related planning (no ClickUp or other platforms).`;
  ...
  async analyzeActionItemsWithContext(actionItems, meetingId) {
    try {
      console.log(
        `[MeetingSummarizer] Analyzing action items with Linear context (meetingId: ${meetingId})`
      );
      logger.logMeetingProcessing(
        "analysis_started",
        "Analyzing action items with Linear context",
        { meetingId }
      );
...
`;
      ...
      console.error(
        `[MeetingSummarizer] Error analyzing action items with context (meetingId: ${meetingId}):`,
        error
      );
      logger.logMeetingProcessing(
        "analysis_error",
        "Error analyzing action items",
        {
          meetingId,
          error: error.message,
        }
  ...
  async createMeetingSummary(transcript) {
    try {
      ...
      console.log(
        `[MeetingSummarizer] Creating meeting summary for transcript (length: ${formattedTranscript.length})`
      );
      logger.logMeetingProcessing(
      ...
      console.log(
        "[MeetingSummarizer] Summary generated successfully (length:",
      ...
      logger.logMeetingProcessing("summary_error", "Error creating summary", {
        error: error.message,
      });
      return {
        success: false,
  ...
  async extractActionItems(summary, meetingId) {
    try {
      // Log full summary to file, truncated to console
      console.log(
        `[MeetingSummarizer] Extracting action items from summary (meetingId: ${meetingId}, length: ${summary.length})`
      );
      logger.logMeetingProcessing(
        "action_items_extraction",
        "Extracting action items from summary",
        {
          meetingId,
      ...
      console.log(
        "[MeetingSummarizer] Action items extracted successfully (length:",
        result.content.length,
        ")"
      );
      logger.logMeetingProcessing(
      ...
      console.error(
        `[MeetingSummarizer] Error extracting action items (meetingId: ${meetingId}):`,
        error
      );
      logger.logMeetingProcessing(
        "action_items_error",
  ...
  async processCompleteWorkflow(transcript, meetingId) {
    try {
      console.log(
        `[MeetingSummarizer] Processing complete workflow for meeting ${meetingId}`
      );
      ...
      const actionItemsResult = await this.extractActionItems(
        summaryResult.summary,
        meetingId
      );
      if (!actionItemsResult.success) {
        console.error(
          `[MeetingSummarizer] Error extracting action items in workflow (meetingId: ${meetingId}):`,
      ...
      return {
        ...
        actionItems: actionItemsResult.success
          ? actionItemsResult.actionItems
          : null,
        actionItemsError: actionItemsResult.success
          ? null
          : actionItemsResult.error,
      };
    } catch (error) {
      console.error(
        "[MeetingSummarizer] Error in complete workflow:",
        error,
        `(meetingId: ${meetingId})`
      );
      return {
