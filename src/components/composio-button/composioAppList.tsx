...
export function ComposioAppList() {
  ...
  const [toolsByApp, setToolsByApp] = useState<Map<string, ComposioTool[]>>(new Map());
  const [selectedTools, setSelectedTools] = useState<Map<string, Set<string>>>(new Map());
  const [loadingTools, setLoadingTools] = useState<Set<string>>(new Set());

  const toolset = useMemo(() => new ComposioToolSet({
  ...
  const categories = useMemo(() => {
    ...
    apps.forEach(app => {
      if (app.categories && app.categories !== "Uncategorized") {
        app.categories.split(", ").forEach(cat => uniqueCategories.add(cat.trim()));
      }
    });
  ...
  const fetchAllConnectionStatuses = useCallback(async (apps: App[]) => {
    ...
    await Promise.all(apps.map(async (app) => {
      try {
        const connection = await entity.getConnection({ app: app.name.toLowerCase() });
        ...
        if (errorCount > maxErrorsAllowed) {
          console.warn("⚠️ Too many connection errors. Halting further checks to avoid spamming logs.");
          return;
        }
        statusMap.set(app.name, "INACTIVE");
      }
  ...
  const fetchToolsForApp = useCallback(async (appName: string) => {
    ...
    try {
      const actions = await toolset.actions.list({ apps: appName.toLowerCase() });
  
      const tools: ComposioTool[] = (actions.items || []).map((action: any) => ({
  ...
  const toggleAppExpansion = useCallback(async (appName: string) => {
  ...
  const toggleToolSelection = useCallback((appName: string, toolName: string) => {
  ...
  useEffect(() => {
    const fetchApps = async () => {
      try {
        ...
        const sortedApps = simplifiedApps.sort((a, b) => a.name.localeCompare(b.name));
        setApps(sortedApps);
  ...
  const filteredApps = useMemo(() => {
    return apps.filter(app => {
      ...
      const matchesConnection = 
        ...
        (connectionFilter === "connected" && appConnectionStatus === "ACTIVE") ||
        (connectionFilter === "not-connected" && appConnectionStatus !== "ACTIVE");

      return matchesSearch && matchesCategory && matchesConnection;
    });
  }, [apps, searchTerm, selectedCategory, connectionFilter, connectionStatuses]);
      ...
      setTimeout(async () => {
        try {
          ...
          setConnectionStatuses(prev => {
            const newMap = new Map(prev);
            newMap.set(appName, connection?.status === "ACTIVE" ? "ACTIVE" : "INACTIVE");
            return newMap;
          });
          ...
          console.log(`Could not check connection status for ${appName}:`, error);
        } finally {
                ...
                <div className="connection-controls">
                  {isConnected && (
                    <button
                      ...
                      title={isExpanded ? "Collapse tools" : "Show available tools"}
                    >
                      ▼
                    </button>
                  )}
                  ...
                  <div className="connection-toggle">
                    {isConnected ? (
                      ...
                      <div 
                        className={`toggle-switch ${isProcessing ? "processing" : ""}`}
                        onClick={() => !isProcessing && handleConnect(app.name)}
                      >
                        <div className="toggle-slider"></div>
                        {isProcessing && <span className="processing-text">Connecting...</span>}
                      </div>
                    )}
                  </div>
                </div>
                        ...
                        <div key={tool.name} className="tool-item">
                          <label className="tool-checkbox">
                            <input
                              ...
                              onChange={() => toggleToolSelection(app.name, tool.name)}
                            />
